...
def MousePosition(MediaPlayer, position):
    if position.y() > QApplication.desktop().screenGeometry().height()-50 or position.y() < 50:
...
def fullscreen(MediaPlayer):
    if MediaPlayer.firstTime_fullscreen:
...
def Remove_from_layout(MediaPlayer):
    ...
    # Create temporary Label to fill gaps
...
def Add_to_layout(MediaPlayer):
    ...
    MediaPlayer.horizontalLayout_5.addWidget(MediaPlayer.pushButton_Tag_of_file,15,QtCore.Qt.AlignRight)
