    ...
    try:
        if val:
            if float(val):
                ...
                if float(val) >= 0 and float(val) <= 3:  # speed rate must be between 0,3
                    SettingWindow.label_Error_Range.setVisible(False)
                    SettingWindow.horizontalSlider_Speed.setValue(
                        float(val)*100)
                    if SettingWindow.MediaPlayer.player.isAvailable():
                        SettingWindow.MediaPlayer.player.setPlaybackRate(
                            float(val))
                else:
                    SettingWindow.label_Error_Range.setVisible(True)

    except:  # Error in enter number
